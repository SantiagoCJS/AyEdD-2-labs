...
int card_drop_points(card fst, card snd, bool isFlush, bool upwardsFlush)
{
    ...
    // sigue la escalera ascendente en sentido ascendente
    if (isFlush && upwardsFlush && (card_number(fst) == card_number(snd) + 1))
    {
        points = 3;
    }
    // sigue la escalera descendente en sentido descendente
    if (isFlush && !upwardsFlush && (card_number(fst) == card_number(snd) - 1))
    {
        points = 3;
    }
    // sigue la escalera ascendente en sentido descendente
    if (isFlush && upwardsFlush && (card_number(fst) == card_number(snd) - 1))
    {
        points = 5;
    }
    // sigue la escalera descendente en sentido ascendente
    if (isFlush && !upwardsFlush && (card_number(fst) == card_number(snd) + 1))
    {
        points = 5;
    }
